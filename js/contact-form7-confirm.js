jQuery(document).ready(function(){
		...
		pages : [
			{
				...
				'button' : {
					...
					'confirm' : '<input type="button"  value="確認" class="button-confirm"/>',	//	html
					'rewrite' : '<input type="button"  value="戻る" class="button-rewrite"/>'	//	html
				}
			}
		],
		validates : {
	...
	.each(function(){
		...
		.click(function(){
			...
			form.find('.reset-button').show();
			jQuery('.label').show();
			jQuery('.message_text').html('下記内容をご記入の上、「確認」ボタンをクリックしてください。');
		...
		.click(function(){
			...
			.each(function(){
				...
				if (child.hasClass('wpcf7-text')){
					if (child.hasClass('wpcf7-validates-as-required') && child.val().length == 0){
						...
						error.append(jQuery('<li>').text(option.validates.email.before + title.replace('*', '') + option.validates.email.after));
						jQuery(this).parents('tr').addClass('error');
					}
				} else if (child.get(0).tagName.toLowerCase() == 'textarea'){
					if (child.hasClass('wpcf7-validates-as-required') && child.val().length == 0){
			...
			if (error.children().length > 0){
				...
				form.find('.reset-button').hide();
				jQuery('.label').hide();
				form.find('.buttons-area').show();
				form.find('.submit-button').show();
				jQuery('.message_text').html('下記ご入力内容をご確認の上、「送信」ボタンをクリックしてください。');
			}
			return false;
		});
